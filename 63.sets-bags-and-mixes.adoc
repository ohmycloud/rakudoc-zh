...
让我们详细说明一下。如果要收集容器中的对象但不关心这些对象的顺序，Perl 6 提供*无序*集合类型 link:https://docs.perl6.org/type/Set[Set], link:https://docs.perl6.org/type/SetHash[SetHash], link:https://docs.perl6.org/type/Bag[Bag], link:https://docs.perl6.org/type/BagHash[BagHash], link:https://docs.perl6.org/type/Mix[Mix], 和 link:https://docs.perl6.org/type/MixHash[MixHash]. 由于无序，这些容器可以比 link:https://docs.perl6.org/type/List[Lists] 更有效地查找元素或处理重复的项目。

另一方面，如果你想获得包含的对象（元素）**而没有重复**并且你只关心元素*是否*在集合中，你可以使用  link:https://docs.perl6.org/type/Set[Set] 或 link:https://docs.perl6.org/type/SetHash[SetHash]。如果你想消除重复但仍保留顺序，请查看 link:https://docs.perl6.org/type/List[List] 的  link:https://docs.perl6.org/routine/unique[unique] 例程。

如果你想跟踪**每个对象出现的次数**，你可以使用 link:https://docs.perl6.org/type/Bag[Bag] 或 link:https://docs.perl6.org/type/BagHash[BagHash]。在这些Baggy 容器中，每个元素都有一个权重（无符号整数），表示同一个对象已包含在集合中的次数。

类型 link:https://docs.perl6.org/type/Mix[Mix] 和  link:https://docs.perl6.org/type/MixHash[MixHash] 类似于 link:https://docs.perl6.org/type/Bag[Bag] 和 link:https://docs.perl6.org/type/BagHash[BagHash]，但它们也允许**分数和负权重**。

link:https://docs.perl6.org/type/Set[Set]，link:https://docs.perl6.org/type/Bag[Bag] 和 link:https://docs.perl6.org/type/Mix[Mix]  是 *immutable* 类型。如果要在构造容器后添加或删除元素，请使用可变变体 link:https://docs.perl6.org/type/SetHash[SetHash], link:https://docs.perl6.org/type/BagHash[BagHash], 和 link:https://docs.perl6.org/type/MixHash[MixHash] 。

六个集合类 `Set`，`SetHash`，`Bag`，`BagHash`，`Mix`，`MixHash`，都有相似的语义。

首先，就它们而言，相同的对象引用相同的元素 - 其中使用 link:https://docs.perl6.org/routine/WHICH[WHICH] 方法确定身份（即以相同的方式link:https://docs.perl6.org/routine/===[===] 运算符检查身份）。对于像 `Str` 这样的值类型，这意味着具有相同的值; 对于像“Array”这样的引用类型，它意味着引用相同的对象实例。
...
有几个中缀运算符致力于在 link:https://docs.perl6.org/type/Set[Set] 上执行常见操作，例如并集和差集。其他操作包括布尔检查，例如对象是否是 `Set` 中的元素，或者一个 `Set` 是否是另一个 `Set` 的子集。

这些中缀可以使用代表函数的 UTF-8 字符编写（如 link:https://docs.perl6.org/routine/%E2%88%88[∈] 或 link:https://docs.perl6.org/routine/%E2%88%AA[∪]），或与 link:https://docs.perl6.org/routine/(elem[(elem)]) 或 link:https://docs.perl6.org/routine/%E2%8A%96[(^)]。

大多数情况下，显式地使用带有这些中缀的 `Set` 对象是不必要的。所有中缀运算符都将处理 link:https://docs.perl6.org/type/Any[Any] 类型的任何对象的参数（例如， link:https://docs.perl6.org/type/List[List]），link:https://docs.perl6.org/type/Array[Array]，link:https://docs.perl6.org/type/Mix[Mix] 等）并强制他们到在需要的地方设置。

在某些情况下，如果参数的类型是 link:https://docs.perl6.org/type/Bag[Bag]，则中缀运算符将以与其行为方式不同但类似的方式运行 `Set` 参数。
...
等价于 link:https://docs.perl6.org/routine/(elem[(elem)]), 在代码点 U+2208 (ELEMENT OF).
...
等价于 link:https://docs.perl6.org/routine/(cont[(cont)]), 在代码点 U+220B (CONTAINS AS MEMBER).
...
等价于 link:https://docs.perl6.org/routine/(%3C=[(<=)]), 在代码点 U+2286 (SUBSET OF OR EQUAL TO).
...
等价于 link:https://docs.perl6.org/routine/(%3C[(<)]), 在代码点 U+2282 (SUBSET OF).
...
像 link:https://docs.perl6.org/routine/(%3C=[(<=)]) 但是翻转参数。 返回 `True` 如果 `$a` 是 `$b` 的**超集**(superset) 或与 `$b` 相等。
...
等价于 link:https://docs.perl6.org/routine/(%3E=[(>=)]), 在代码点 U+2287 (SUPERSET OF OR EQUAL TO).
...
像 link:https://docs.perl6.org/routine/(%3C[(<)]) 但是反转参数。返回 `True` 如果 `$a` 是 `$b` 的一个**严格超集**(strict superset)。
...
等价于 link:https://docs.perl6.org/routine/(%3E[(>)]), 在代码点 U+2283 (SUPERSET OF).
...
等价于 link:https://docs.perl6.org/routine/(|[(|)]), 在代码点 U+222A (UNION).
...
等价于 link:https://docs.perl6.org/routine/(&[(&)]), 在代码点 U+2229 (INTERSECTION).
...
等价于 link:https://docs.perl6.org/routine/(-[(-)]).
...
等价于 link:https://docs.perl6.org/routine/($CIRCUMFLEX_ACCENT[(^)]), 在代码点 U+2296 (CIRCLED MINUS).
...
等价于link:https://docs.perl6.org/routine/(.[infix (.)]), 在代码点 U+228D (MULTISET MULTIPLICATION).
...
等价于 link:https://docs.perl6.org/routine/(+[(+)]), 在代码点 U+228E (MULTISET UNION).

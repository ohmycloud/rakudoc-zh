...
您可以使用 link:https://docs.perl6.org/routine/sink-all[`sink-all`] 方法在 link:https://docs.perl6.org/type/Iterator[Iterator] 上强制使用该接收器上下文。link:https://docs.perl6.org/type/Proc[Proc]也可以通过 link:https://docs.perl6.org/type/Proc#method_sink[`sink`] 方法沉没，迫使它们引发异常而不返回任何东西。

通常，如果在 sink 上下文中进行计算，则块将发出警告; 但是，在 sink 上下文中 link:https://docs.perl6.org/language/control#Flow%2529_gather_take[gather/take] 块是显式计算的，并使用 `take` 显式返回值。
...
这个上下文，可能除了上面的所有内容之外，都是*转换*或*解释*上下文，因为它们接收无类型或类型化的变量，并将其类型化为执行操作所需的任何内容。在某些情况下，这意味着转换（例如从 link:https://docs.perl6.org/type/Str[Str] 到 link:https://docs.perl6.org/type/Numeric[Numeric]）; 在其他情况下只是一种解释（link:https://docs.perl6.org/type/IntStr[IntStr] 将被解释为 link:https://docs.perl6.org/type/Int[Int] 或 link:https://docs.perl6.org/type/Str[Str]）。
...
可以使用算术运算符强制数字上下文，例如 `+` 或 `-`。在该上下文中，link:https://docs.perl6.org/routine/Numeric[`Numeric`] 将调用该方法（如果可用），并将返回的值用作对象的数值。
...
在*列表*那样的东西的情况下，数值通常等于 `.elems`; 在某些情况下，像link:https://docs.perl6.org/routine/Numeric#%28Thread%29_method_Numeric[Thread] 一样，它将返回唯一的线程标识符。
...
通常，将在变量上调用 link:https://docs.perl6.org/routine/Str[`Str`] 例程以将其上下文化; 因为这个方法是从 link:https://docs.perl6.org/type/Mu[Mu] 继承的，所以它始终存在，但并不总能保证工作。在某些核心类中，它会发出警告。

link:https://docs.perl6.org/routine/~[`~`] 是（一元）字符串上下文化器。作为运算符，它连接字符串，但作为前缀运算符，它成为字符串上下文运算符。
...
当 `link:https://docs.perl6.org/language/operators#Reduction_operators[~]` 应用于列表时，这也将在 [*reduction*] 上下文中发生：
...
由于 link:https://docs.perl6.org/routine/~#%28Operators%29_infix_~[`~`] 也作为缓冲区连接运算符，因此必须检查每个元素是否为空，因为字符串上下文中的单个空缓冲区将表现为字符串，从而产生错误。
...
由于 `~` 将字符串上下文放入此列表的第二个元素，link:https://docs.perl6.org/routine/~#%28Operators%29_infix_~[`~`] 将使用适用于字符串的第二个形式，从而产生所显示的错误。只需确保连接的所有内容都是缓冲区即可避免此问题。

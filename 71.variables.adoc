...
不带符号的变量也是可行的, 查看 link:http://doc.raku.org/language/variables#Sigilless_variables[无符号变量].
...
例如, link:https://docs.raku.org/type/Array[数组]变量(@符号)在列表赋值时清空数组自身, 然后把等号右侧的值都放进数组自身中. 跟 Item 赋值相比, 这意味着等号左侧的变量类型始终是 Array, 不管右侧是什么类型.
...
查看link:https://docs.raku.org/language/operators[操作符]获取关于优先级的更多详情。
...
然而, 这些无符号变量并不创建link:https://docs.raku.org/language/containers[容器]. 那意味着上面的 `degrees` 和 `θ` 实际上直接代表 `Nums`. 为了说明, 我们定义一个无符号变量后再赋值:
...
动态变量与其他变量类型在引用一个未声明的动态变量上不同的是前者不是一个编译时错误，而是运行时 link:https://docs.raku.org/type/Failure[failure]，这样一个动态变量可以在未定义时使用只要在把它用作任何其它东西的时候检查它是否定义过:
...
注意属性是怎样被声明为 `$.x` 和 `$.y` 的, 但是仍然能够通过 `$!x` 和 `$!y` 访问到属性. 这是因为 在 Raku 中, 所有的属性都是`私有的`, 并且在类中能使用 `$!attribute-name` 直接访问这些属性. Raku 能自动为你生成访问方法. 关于对象、类和它们的属性和方法的详情, 请查看link:https://docs.raku.org/language/objects[面向对象].
...
关于这些特殊变量的列表请查看link:https://docs.raku.org/language/variables#Compile-time_variables[编译时变量]。
...
关于对象、类和它们的属性和方法的详情, 请查看link:https://docs.raku.org/language/objects[面向对象].
...
查看 link:https://docs.raku.org/routine/%5E[^]获取关于占位符变量的更多细节。
...
然而, 有很多声明符能在 link:https://docs.raku.org/language/variables#Twigils[Twigils] 的能力之外改变作用域的细节。
...
为了让 `new-location()` 能打印 `nowwhere`, 需要使用 `*` link:https://docs.raku.org/language/variables#The_%2A_Twigil[twigil] 让 `$location` 变为动态变量.
...
查看link:https://docs.raku.org/language/objects[面向对象]获取更多文档和例子。
...
`$/` 是匹配变量。它存储着最近一次link:https://docs.raku.org/language/regexes[正则]匹配的结果，通常包含 link:https://docs.raku.org/type/Match[Match] 类型的对象。

...
定义自定义操作符在 link:http://doc.perl6.org/language/functions#Defining_Operators[ /language/functions#Defining_Operators.]  中有涉及。
...
查看 link:http://doc.perl6.org/language/subscripts[Subscripts] 获取关于该操作符行为的更详细的解释， 还有怎么在自定义类型中实现对它的支持。
...
可变的方法调用。 `$invocant.=method` ， 脱去语法糖后就是 `$invocant = $invocant.method` , 这与 link:http://doc.perl6.org/routine/op%3D[op=.] 类似。
...
元方法调用。 ` $invocant.^method` 在  `$invocant` 的元类身上调用方法。脱去语法糖后， 它就是 `$invocant.HOW.method($invocant, ...)` 。 查看 link:http://doc.perl6.org/type/HOW[HOW] 获取更多信息。

...
- **Positional** 下标 (通过 link:http://doc.perl6.org/language/operators#postcircumfix_[_][postcircumfix [ ]] 通过元素在有序集合中的位置来寻址元素。)索引 0 引用第一个元素， 索引 1 引用第二个元素， 以此类推：
...
- **Associative** 下标 (通过 link:http://doc.perl6.org/language/operators#postcircumfix_{_}[postcircumfix { }]), 不要求集合以任何特定的顺序保存元素 - 相反，它使用一个唯一的键来寻址每个值。键的种类取决于使用的集合： 举个例子， 一个标准的link:http://doc.perl6.org/type/Hash[散列] 使用字符串作为键， 而一个 link:http://doc.perl6.org/type/Mix[Mix] 能使用任意的对象作为键， 等等:
...
```
  ...
  相对于传递单个-单词字符串键给 `{ }` , 你也可以使用link:http://doc.perl6.org/language/quoting#Word_quoting:_qw[以尖括号引起单词的结构] ，就像它们是后缀操作符一样：
...
当通过下标寻址一个不存在的元素所发生的事情取决于正在使用的集合类型。标准的 Array 和 Hash 集合返回它们的link:http://doc.perl6.org/routine/of[value type constraint]  的类型对象（这默认是 `Any`）。
...
为了在下标操作中默默地跳过不存在的元素， 查看 link:http://doc.perl6.org/language/subscripts#Truncating_slices[#Truncating slices]  和 link:http://doc.perl6.org/language/subscripts#%3Av[`#:v`] 副词。
...
这里实际发生的是， `*-1` 那样的表达式通过 link:http://doc.perl6.org/type/Whatever[Whatever] 柯里化声明了一个代码对象 - `[ ]`会把代码对象作为索引， 通过集合的长度作为参数来调用它并使用结果值作为实际的索引。 换句话说，`@alphabet[*-1]` 变成了 `@alphabet[@alphabet.elems - 1]`。
...
对于 positional  切片， 你可以混合普通切片和  link:http://doc.perl6.org/language/subscripts#From_the_end[from-the-end] 切片：
...
要知道切片是由传入 (link:http://doc.perl6.org/language/subscripts#Multiple_dimensions[one dimension of])下标的类型控制的，而非它的长度：
...
(尖括号形式的 associative 下标也没有问题，因为 link:http://doc.perl6.org/language/quoting#Word_quoting:_qw[word quoting]  在单个单词的情况下很方便的返回一个 Str, 但是在多个单词的情况下返回一个 link:http://doc.perl6.org/type/Parcel[Parcel])。

对于普通的切片，下标的内容 (link:http://doc.perl6.org/language/subscripts#Multiple_dimensions[the current dimension of]) 在它的元素被解释为 `索引/键` 之前会被展平(flattened)：
...
link:http://doc.perl6.org/language/subscripts#From_the_end[From-the-end]  索引被允许作为范围的端点，代表无限的范围和序列：
...
如果你不想把你的切片指定为 `range/sequence` 但仍旧想默默地跳过不存在的元素， 你可以使用 link:http://doc.perl6.org/language/subscripts#%3Av[#:v] 副词。
...
查看 link:http://doc.perl6.org/language/subscripts#method_BIND-POS[#method BIND-POS] 和 link:http://doc.perl6.org/language/subscripts#method_BIND-KEY[#method BIND-KEY] 了解底层机制.
...
要检查切片的所有元素是否存在， 使用 link:http://doc.perl6.org/routine/all[all] junction:

```perl6
if all %fruit<apple orange banana>:exists { ... }
```

`:exists`  可以和  link:http://doc.perl6.org/language/subscripts#%3Adelete[:delete] 还有 `:p/:kv` 副词组合 - 这时表达式的行为就由那些副词决定，除了使用表明元素存在的对应 Bool 值替换返回的元素值之外。

查看 link:http://doc.perl6.org/language/subscripts#method_EXISTS-POS[method EXISTS-POS]  和 link:http://doc.perl6.org/language/subscripts#method_EXISTS-KEY[method EXISTS-KEY] 了解底层机制.
...
查看 link:http://doc.perl6.org/language/subscripts#method_DELETE-POS[method DELETE-POS] and link:http://doc.perl6.org/language/subscripts#method_DELETE-KEY[method DELETE-KEY] 了解底层机制.
...
也可以查看  link:http://doc.perl6.org/routine/pairs[pairs] 子例程.
...
也可以查看  link:http://doc.perl6.org/routine/kv[kv] 子例程.
...
还可以查看  link:http://doc.perl6.org/routine/keys[keys] 子例程.
...
还可以查看  link:http://doc.perl6.org/routine/values[values] 子例程.
...
你不必手动重载  link:http://doc.perl6.org/routine/[%20]#postcircumfix_[_][postcircumfix [ ]] 和  link:http://doc.perl6.org/[postcircumfix { }]  操作符并重新实现它们所有的戏法， 为了实现它， 相反，你可以依赖这个事实， 在幕后，它们的标准实现分派给了一个定义良好的底层方法集。例如：
...
所以， 为了让你的下标工作， 你只需要为你的自定义类型实现或委托那些底层方法（link:http://doc.perl6.org/language/subscripts#Methods_to_implement_for_positional_subscripting[下面描述详情]）。

如果你这样做了， 你还应该让你的类型各自遵守 link:http://doc.perl6.org/type/Positional[Positional] 或 link:http://doc.perl6.org/type/Associative[Associative] role
...
为了通过  link:http://doc.perl6.org/routine/[%20]#postcircumfix_[_][postcircumfix [ ]]  让基于索引的下标在你的自定义类型中工作，你应该至少实现下面的  `elems`, `AT-POS` 和 `EXISTS-POS-` 还有其它可选项。
